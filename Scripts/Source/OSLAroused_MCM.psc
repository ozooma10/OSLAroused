...
function SettingsLeftColumn()
    ...
    MinLibidoOid = AddSliderOption("Minimum Libido", Main.MinLibidoValue, "{1}")
...
event OnOptionHighlight(int optionId)
    ...
    elseif(CurrentPage == "Settings")
        ...
        elseif(optionId == MinLibidoOid)
            SetInfoText("Minimum Value Libido can fall to. This can also be used to set the value arousal should rise towards over time")
...
event OnOptionSliderOpen(int option)
    if(CurrentPage == "Puppeteer")
        ...
        elseif (option == SetLibidoOid)
            ...
            SetSliderDialogDefaultValue(Main.MinLibidoValue)
            SetSliderDialogRange(Main.MinLibidoValue, 100)
            SetSliderDialogInterval(1)
    ...
    elseIf (currentPage == "Settings")
        if(option == MinLibidoOid)
            SetSliderDialogStartValue(Main.MinLibidoValue)
...
event OnOptionSliderAccept(int option, float value)
    ...
    elseIf (currentPage == "Settings")
        if(option == MinLibidoOid)
            Main.SetMinLibidoValue(value)
            SetSliderOptionValue(MinLibidoOid, value, "{1}")
        elseif(option == SceneParticipantBaselineOid)
...
event OnOptionDefault(int option)
    if(currentPage == "Puppeteer")
        ...
        elseif(option == SetLibidoOid)
            OSLArousedNative.SetLibido(PuppetActor, Main.MinLibidoValue)
            SetSliderOptionValue(SetLibidoOid, Main.MinLibidoValue, "{1}")
        elseif(option == SetArousalMultiplierOid)
    ...
    elseif(CurrentPage == "Settings")
        if(option == MinLibidoOid)
            Main.SetMinLibidoValue(50)
            SetSliderOptionValue(MinLibidoOid, 50, "{1}")
        elseif(option == SceneParticipantBaselineOid)
