...
- Git submodules initialized: `git submodule update --init --recursive` (pulls `lib/commonlibsse` and its `commonlib-shared`)

CommonLibSSE is vendored as the `lib/commonlibsse` git submodule (the libxse fork, which supports xmake natively). There is no longer a vcpkg or CMake dependency.
...
- `xmake install` deploys to `XSE_TES5_MODS_PATH` or `XSE_TES5_GAME_PATH` if set (handled by the `commonlibsse.plugin` rule)
- The SKSE plugin version/metadata is generated by the `commonlibsse.plugin` rule (no manual `SKSEPlugin_Version` in source); read at runtime via `SKSE::PluginVersionData::GetSingleton()`
